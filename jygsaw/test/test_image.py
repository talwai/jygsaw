...
class TestImage(unittest2.TestCase):
    def setUp(self):
        ...
        self.valid_path = path.join(path.dirname(path.abspath(
            getfile(currentframe()))), "puppy_for_test_image.jpg")
    ...
    def test_check_valid_url(self):
        self.assertFalse(self.i.check_valid_url(
            self.valid_path), msg="Valid File path should return false")

        # Invalid path should thrown an exception
        with self.assertRaises(Exception):
            self.i.check_valid_url(self.invalid_path)

        self.assertTrue(self.i.check_valid_url(
            self.valid_url), msg="valid URL should return True")
    ...
    def test_path_attribute(self):
        self.assertEqual(self.i.path, self.valid_path,
                         msg="Attribute path is set incorrectly")
    ...
    def test_width_attribute(self):
        ...
        self.i.width = 100
        self.assertEqual(
            self.i.width, 100, msg="Was not able to set attribute width")
    ...
    def test_height_attribute(self):
        self.assertEqual(
            self.i.height, None, msg="Attribute height should be none")
