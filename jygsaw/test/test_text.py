...
class TestTextClass(unittest2.TestCase):
    ...
    def test_string_attribute(self):
        self.assertEqual(self.text.s, self.string,
                         msg="Default string in text object not correct")

        self.newString = "New string"
        self.text.s = self.newString

        self.assertEqual(self.text.s, self.newString,
                         msg="Was not able to change string in text object")
    ...
    def test_size_attribute(self):
        self.assertEqual(
            self.text.size, 12, msg="Default size in text object not correct")

        self.newSize = 20
        self.text.size = self.newSize

        self.assertEqual(self.text.size, self.newSize,
                         msg="Was not able to change size in text object correctly")
    ...
    def test_attributes_attribute(self):
        self.assertEqual(self.text.attribute, PLAIN,
                         msg="Default attribute in text object not correct")

        self.newAttribute = BOLD
        self.text.attribute = self.newAttribute

        self.assertEqual(self.text.attribute, self.newAttribute,
                         msg="Was not able to change attribute in text object")
